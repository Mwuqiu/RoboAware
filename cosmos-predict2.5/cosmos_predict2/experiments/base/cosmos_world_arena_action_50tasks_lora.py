...
#
# Action-conditioned LoRA training on the full WorldArena RoboTwin 50-task set.
# Mirrors the video-only LoRA 50-task baseline (same data root / frames / res /
# batch / LR schedule / LoRA rank) but injects the 14-dim dual-arm relative-joint
# action via the CHUNK action net (per-latent-frame AdaLN modulation).
# Trainables = LoRA adapters + action embedders (unfrozen in
# ActionVideo2WorldModelRectifiedFlow.set_up_model). Backbone base weights frozen.
# No text conditioning (zero t5), matching the official Bridge action recipe.
...
DATASET_ROOT = "/root/autodl-tmp/cosmos_training_data_world_arena"  # full 50-task set
NUM_FRAMES = 121
NUM_ACTION_PER_CHUNK = NUM_FRAMES - 1  # 120 relative deltas; chunk net groups 4 per latent frame
ACTION_DIM = 14
...
dataloader_train = L(get_generic_dataloader)(
    ...
    batch_size=2, drop_last=True, num_workers=12, pin_memory=True,  # 4xA800 -> effective 8
)
...
dataloader_val = L(get_generic_dataloader)(
    ...
    batch_size=2, drop_last=True, num_workers=4, pin_memory=True,
)

# Local .pt base (NOT s3.uri DCP dir -> loader would silently load nothing -> loss ~3.0).
# strict_resume=False: action embedder + LoRA keys do not exist in the base checkpoint.
BASE_CKPT = (
...
EXPERIMENT_NAME = "predict2_action_2b_world_arena_50tasks_lora"
...
experiment_config = dict(
    ...
    job=dict(project="cosmos_predict_v2p5", group="action", name="action_50tasks_lora_480x640"),
    ...
    # Optimizer/scheduler identical to the video-only LoRA baseline: LR curve over the
    # first 6k iters matches that run exactly.
    optimizer=dict(lr=2 ** (-14.5), weight_decay=0.001),
    scheduler=dict(f_max=[0.5], f_min=[0.2], warm_up_steps=[2_000], cycle_lengths=[100000]),
    trainer=dict(
        run_validation=False, logging_iter=100, max_iter=6000,
        callbacks=dict(
            heart_beat=dict(save_s3=False), iter_speed=dict(hit_thres=200, save_s3=False),
    ...
    model=dict(config=dict(
        # LoRA identical to video-only baseline
        use_lora=True,
        lora_rank=32,
        lora_alpha=32,
        lora_target_modules="q_proj,k_proj,v_proj,output_proj,mlp.layer1,mlp.layer2",
        init_lora_weights=True,
        # action-conditioned setup (matches official Bridge recipe: 1 cond frame)
