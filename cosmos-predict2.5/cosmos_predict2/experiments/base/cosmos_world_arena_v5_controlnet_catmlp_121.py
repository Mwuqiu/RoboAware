...
#
# V5 ControlNet experiment: cross_attn_then_mlp adapter (cross-attn + MLP, NO self-attn) + ControlNet weight copy
...
#
# Diffs vs v5_controlnet_10tasks:
#   - full /root/autodl-tmp/cosmos_training_data_world_arena dataset
#   - num_frames=121
#   - point_adapter_adapter_mode="cross_attn_then_mlp": adapter runs cross-attn + MLP,
#     and SKIPS self-attn. (self-attn is a PC-independent bypass that drove the
#     fullsa_121 left-arm bias; PC-swap showed fullsa ignored PC, catmlp uses it.)
...
trainer_conf = dict(
    ...
    logging_iter=25,
    max_iter=10000,    # post-bug-fix default
    callbacks=dict(
...
model_conf = dict(
    config=dict(
        ...
        net=dict(
            ...
            point_adapter_adapter_mode="cross_attn_then_mlp",   # D4-Bprime: cross-attn + MLP, NO self-attn (closes PC-independent bypass)
...
EXPERIMENT_NAME = "predict2_point_adapter_v5_controlnet_catmlp_121_worldarena"
...
experiment_config = dict(
    ...
    job=dict(
        ...
        group="point_adapter",
        name="v5_controlnet_catmlp_121_worldarena_480x640",
    ),
