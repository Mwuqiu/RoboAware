...
@dataclass(frozen=True)
class PCVideo2WorldCondition(Text2WorldCondition):
    use_video_condition: bool = False
    # the following two attributes are used to set the video condition; during training, inference
    gt_frames: Optional[torch.Tensor] = None
    condition_video_input_mask_B_C_T_H_W: Optional[torch.Tensor] = None
...
class PCVideo2WorldConditioner(GeneralConditioner):
    def forward(
        self,
        batch: Dict,
        override_dropout_rate: Optional[Dict[str, float]] = None,
    ) -> PCVideo2WorldCondition:
        output = super()._forward(batch, override_dropout_rate)
        return PCVideo2WorldCondition(**output)
...
PointPredictionConditioner: LazyDict = L(PCVideo2WorldConditioner)(
    **_Point_CONFIG,
)
...
def register_conditioner():
    ...
    cs.store(
        group="conditioner",
        package="model.config.conditioner",
        name="pc_video_prediction_conditioner",
        node=PointPredictionConditioner,
    )
