#!/usr/bin/env python3
"""Run D4-CN inference TWICE with online PC encoding on the same input.

Diffusion seed is fixed; only PC encoding is re-randomized (shuffle_orders).
If output mp4s are close → adapter is robust to shuffle nondeterminism.
"""
...
def main():
    ...
    p.add_argument("--output-dir", default="/root/autodl-tmp/online_diff_test")
    ...
    # Online PC encoding (re-runs PTV3 each call)
    ...
    dataset = VideoDataset(
        ...
        pc_feat_input="coord",
        pc_extract_layer="dec_0",
    )

    # Run 1 — first online encode + diffusion (fixed seed 0)
    print(f"\n=== RUN 1: online encode #1 + diffusion (seed=0) ===")
    b1 = build_batch(model, dataset, ds_dir, args.ep_id, args.num_conditional_frames, device)
    pc1 = b1["pc_latent_x0"].detach().clone()
    t0 = time.time()
    v1 = run_one(model, b1, seed=0)
    print(f"  elapsed {time.time()-t0:.1f}s, video.shape={tuple(v1.shape)}")
    save_img_or_video(v1, str(out / "run1.mp4"), fps=30)

    # Run 2 — second online encode + diffusion (same fixed seed 0)
    print(f"\n=== RUN 2: online encode #2 + diffusion (seed=0) ===")
    b2 = build_batch(model, dataset, ds_dir, args.ep_id, args.num_conditional_frames, device)
    pc2 = b2["pc_latent_x0"].detach().clone()
    t0 = time.time()
    v2 = run_one(model, b2, seed=0)
    print(f"  elapsed {time.time()-t0:.1f}s")
    save_img_or_video(v2, str(out / "run2.mp4"), fps=30)

    # Diff analysis
    print(f"\n=== PC feature diff (online encode randomness) ===")
    pc1c = pc1.float().cpu(); pc2c = pc2.float().cpu()
    print(f"  shape: {tuple(pc1c.shape)}")
    print(f"  diff max: {(pc1c - pc2c).abs().max().item():.4f}")
    print(f"  sorted-norms diff max (frame 0): {(torch.linalg.norm(pc1c[0,0],dim=1).sort()[0] - torch.linalg.norm(pc2c[0,0],dim=1).sort()[0]).abs().max().item():.4f}")

    print(f"\n=== Video diff (pixel level, skip cond frame) ===")
    d = (v1[:, 1:] - v2[:, 1:]).abs()
    print(f"  pixel diff mean: {d.mean().item()*255:.3f}/255 = {d.mean().item()*100:.3f}%")
    print(f"  pixel diff p99:  {d.flatten().quantile(0.99).item()*255:.3f}/255")
