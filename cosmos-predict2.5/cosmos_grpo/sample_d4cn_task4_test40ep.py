#!/usr/bin/env python3
"""D4-CN iter_5000 sampling on Task 4 test (all 40 episodes).

Each ep uses meta text as prompt + GT first frame + precomputed PC (dec_0 V5).
Output: 40 mp4 + per-ep first-frame PNG (for later WorldArena eval).
"""
...
def main():
    ...
    p.add_argument("--output-dir", default="/root/autodl-tmp/d4cn_iter5k_task4test_40ep")
    ...
    print(f"Found {len(videos)} test videos in Task 4")
