#!/usr/bin/env python3
"""Sample current fullsa_121 model (iter_6500 EMA) on one episode per 10-task subset.

Same protocol as the d4cn 10tasks eval (meta prompt + GT first frame + precomputed PC),
but num_frames=121 to match the fullsa_121 training, and experiment switched accordingly.
Output: 10 mp4 in /root/autodl-tmp/sample_fullsa121_iter6500/
"""
...
def main():
    ...
    p.add_argument("--ckpt", default="/root/autodl-tmp/v5_fullsa121_iter6500_pt/model_ema_bf16.pt")
    p.add_argument("--experiment", default="predict2_point_adapter_v5_controlnet_fullsa_121_worldarena")
    p.add_argument("--config", default="cosmos_predict2/_src/predict2/configs/video2world/config.py")
    p.add_argument("--dataset-dir", default="/root/autodl-tmp/sample_fullsa121_subset")
    p.add_argument("--output-dir", default="/root/autodl-tmp/sample_fullsa121_iter6500")
    ...
    dataset = VideoDataset(
        dataset_dir=str(ds_dir), num_frames=args.num_frames, video_size=(480, 640),
    ...
    print(f"Found {len(videos)} subset videos")
