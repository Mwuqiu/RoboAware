#!/usr/bin/env python3
"""PC swap inference for D4-CN iter_5000 (10tasks model) — 6 runs.

Three experiment groups:
  Group A: dual-arm init + single-arm PC (probe: does PC kill an arm?)
    A0 frame=pick_dual_40 + PC=pick_dual_40   (baseline DUAL)
    A1 frame=pick_dual_40 + PC=adjust_bottle_40 (R donor)     -> expect single R?
    A2 frame=pick_dual_40 + PC=adjust_bottle_48 (L donor)     -> expect single L?

  Group B: same-task arm flip (move_can_pot)
    B1 frame=move_can_pot_40 (R-task) + PC=move_can_pot_41 (L donor) -> arm flip R->L?
    B2 frame=move_can_pot_41 (L-task) + PC=move_can_pot_40 (R donor) -> arm flip L->R?

  Group B baselines:
    B0_R frame=move_can_pot_40 + PC=move_can_pot_40 (baseline R)
    B0_L frame=move_can_pot_41 + PC=move_can_pot_41 (baseline L)

ckpt = v5_d4cn_10tasks_iter5000_pt (latest)
dataset = 10tasks test
"""
...
def main():
    ...
    EP_ADJ_R = ep("adjust_bottle", "000040")           # R
    EP_ADJ_L = ep("adjust_bottle", "000048")           # L
    EP_MCP_R = ep("move_can_pot", "000040")            # R
    EP_MCP_L = ep("move_can_pot", "000041")            # L

    print("=== build all 5 base batches ===")
    batches = {}
    for tag in [EP_PICK_DUAL, EP_ADJ_R, EP_ADJ_L, EP_MCP_R, EP_MCP_L]:
        print(f"  building {tag}")
        batches[tag] = build(tag)
    ...
    # (tag, init_batch_ep, pc_donor_ep, description)
    runs = [
        ("A0_dualinit_dualPC_baseline",  EP_PICK_DUAL, EP_PICK_DUAL, "dual init + dual PC (baseline)"),
        ("A1_dualinit_RPC",              EP_PICK_DUAL, EP_ADJ_R,     "dual init + R PC (kill L arm?)"),
        ("A2_dualinit_LPC",              EP_PICK_DUAL, EP_ADJ_L,     "dual init + L PC (kill R arm?)"),
        ("B0R_mcpRinit_RPC_baseline",    EP_MCP_R,     EP_MCP_R,     "move_can_pot R init + R PC (baseline)"),
        ("B0L_mcpLinit_LPC_baseline",    EP_MCP_L,     EP_MCP_L,     "move_can_pot L init + L PC (baseline)"),
        ("B1_mcpRinit_LPC_flip",         EP_MCP_R,     EP_MCP_L,     "move_can_pot R init + L PC (flip R->L)"),
        ("B2_mcpLinit_RPC_flip",         EP_MCP_L,     EP_MCP_R,     "move_can_pot L init + R PC (flip L->R)"),
    ]
    for tag, init_ep, pc_ep, desc in runs:
        ...
        print(f"\n=== {tag} | {desc} | init={init_ep} pc={pc_ep} ===")
        t0 = time.time()
    ...
    manifest = "\n".join(f"{tag}\t{init_ep}\t{pc_ep}\t{desc}" for tag, init_ep, pc_ep, desc in runs)
    (out_dir / "manifest.tsv").write_text("tag\tinit_ep\tpc_donor_ep\tdescription\n" + manifest + "\n")
